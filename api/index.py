...
### <<< THAY ĐỔI: Chức năng quản lý công việc thủ công ###
...
def add_task(chat_id, task_string: str) -> str:
    ...
    tasks.append({"time_iso": task_dt.isoformat(), "name": name_part}) # Bỏ cờ "reminded"
...
def list_tasks(chat_id) -> str:
    ...
    if len(active_tasks) < len(user_tasks): # Dọn dẹp task cũ
...
# --- CÁC HÀM CRYPTO VÀ TIỆN ÍCH BOT (Giữ nguyên từ phiên bản ổn định trước) ---
...
# Endpoint /check_reminders đã được xóa bỏ vì không cần thiết nữa.